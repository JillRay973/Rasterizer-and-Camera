...
QImage Rasterizer::RenderScene()
{
    ...
    for(Polygon& p : polygons) {
        for(Vertex& v : p.m_verts) {
            v.m_pos = camera.returnViewMatrix() * v.m_pos;
            v.m_pos = camera.returnProjectionMatrix() * v.m_pos;
        ...
        for(Polygon p : polygons) {
            for(Triangle t : p.m_tris) {
                ...
                if(b[2] > y && b[3] < y) {
                    ...
                    lineSegment l1 = lineSegment(v1, v2);
                    lineSegment l2 = lineSegment(v1, v3);
                    lineSegment l3 = lineSegment(v2, v3);
                    //Finding x intersections
                    if(l1.findIntersection(y, &x1) && x1 >= b[0] && x1 <= b[1]) {
                        if(l2.findIntersection(y, &x2) && x2 >= b[0] && x2 <= b[1]) {
                            if(l3.findIntersection(y, &x3)) {
                                if(glm::abs(x1 - x2) < 0.001) {
                                    x2 = x3;
                                }
                            }
                        } else {
                            l3.findIntersection(y, &x2);
                        }
                    } else {
                        l2.findIntersection(y, &x1);
                        l3.findIntersection(y, &x2);
                    }
                    if(x1 > x2) {
                        x3 = x1;
